...
const OfferCard: React.FC<OfferCardProps> = ({ productName, discountPercentage, className, image, route }): ReactElement => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition duration-300 flex flex-col justify-end p-6">
        ...
        <Link href={route}
          className="mt-4 cursor-pointer text-sm font-semibold text-white border-b-2 border-white self-start pb-1 hover:border-yellow-300 transition-colors"
        >
          SHOP NOW
        </Link>
      </div>
